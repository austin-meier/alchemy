...
fn identify_alchemy_doc_cutline_frames() {
    ...
    let cutline_page1 = doc
        .get_dielines_for_page(0)
        .unwrap_or_else(|| panic!("Failed to find any dielines on page 1"));
    let cutline_page2 = doc
        .get_dielines_for_page(1)
        .unwrap_or_else(|| panic!("Failed to find any dielines on page 2"));
